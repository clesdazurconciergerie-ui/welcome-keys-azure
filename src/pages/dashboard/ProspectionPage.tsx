...
import {
  Plus, Search, Filter, Phone, Mail, MapPin, CalendarIcon, MessageSquare,
  TrendingUp, Users, Target, AlertTriangle, CheckCircle, Clock, ArrowRightLeft,
  Flame, Snowflake, Sun, Star, Trash2, Edit, UserPlus, X, ChevronRight, BarChart3
} from "lucide-react";
import {
    ...
    <div className="space-y-6 max-w-[1600px]">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gérez votre pipeline commercial et convertissez vos prospects</p>
        </div>
        <Button onClick={() => setShowCreateDialog(true)} className="bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700 text-white shadow-lg shadow-violet-200">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: "Prospects actifs", value: kpis.active, icon: Users, gradient: "from-blue-500 to-cyan-500" },
          { label: "Signés ce mois", value: kpis.signedThisMonth, icon: CheckCircle, gradient: "from-emerald-500 to-green-500" },
          { label: "Taux conversion", value: `${kpis.conversionRate}%`, icon: TrendingUp, gradient: "from-violet-500 to-purple-500" },
          { label: "Valeur pipeline", value: `${kpis.pipelineValue.toLocaleString()}€`, icon: BarChart3, gradient: "from-amber-500 to-orange-500" },
          { label: "Relances en retard", value: kpis.overdueFollowups, icon: AlertTriangle, gradient: kpis.overdueFollowups > 0 ? "from-red-500 to-rose-500" : "from-slate-400 to-slate-500" },
        ].map((kpi, i) => (
              ...
              <Droppable key={status.value} droppableId={status.value}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={cn(
                      "min-w-[260px] w-[260px] rounded-2xl p-3 transition-colors",
                      snapshot.isDraggingOver ? "bg-violet-50 ring-2 ring-violet-200" : "bg-muted/40"
                    )}
                  >
                    <div className="flex items-center justify-between mb-3 px-1">
                      <div className="flex items-center gap-2">
                        <span className={cn("px-2 py-0.5 rounded-full text-xs font-semibold border", status.color)}>
                          {status.label}
                        </span>
                      </div>
                      <span className="text-xs font-bold text-muted-foreground bg-background rounded-full w-6 h-6 flex items-center justify-center">
                        ...
                        <Draggable key={prospect.id} draggableId={prospect.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={cn(
                                "bg-background rounded-xl p-3 shadow-sm border border-border/50 cursor-pointer hover:shadow-md transition-all",
                                snapshot.isDragging && "shadow-lg ring-2 ring-violet-300 rotate-2"
                              )}
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <UserPlus className="w-5 h-5 text-violet-500" /> Nouveau prospect
          </DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <div className="flex gap-2 mt-1">
              {WARMTH_LEVELS.map(w => (
                <button key={w.value} onClick={() => setForm(f => ({ ...f, warmth: w.value }))} className={cn("px-3 py-1.5 rounded-full text-sm font-medium transition-all", form.warmth === w.value ? w.color + " ring-2 ring-offset-1" : "bg-muted text-muted-foreground")}>
                  {w.emoji} {w.label}
                </button>
              ))}
            </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} className="bg-gradient-to-r from-violet-500 to-purple-600 text-white">Créer</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
          ...
          <SheetTitle className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className={cn("px-2 py-0.5 rounded-full text-xs font-medium border", statusInfo?.color)}>{statusInfo?.label}</span>
                <span className={cn("px-2 py-0.5 rounded-full text-xs font-medium", warmthInfo?.color)}>{warmthInfo?.emoji} {warmthInfo?.label}</span>
              </div>
            </div>
          </SheetTitle>
        </SheetHeader>
          ...
          <TabsContent value="info" className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl bg-emerald-50 text-center">
                ...
                <p className="text-lg font-bold text-emerald-700">{prospect.estimated_monthly_revenue?.toLocaleString()}€<span className="text-xs font-normal">/mois</span></p>
              </div>
              <div className="p-3 rounded-xl bg-amber-50 text-center">
                <p className="text-xs text-muted-foreground">Score</p>
                <p className="text-lg font-bold text-amber-700 flex items-center justify-center gap-1"><Star className="w-4 h-4 fill-amber-400 text-amber-400" />{prospect.score}</p>
              </div>
            </div>
            <Separator />
