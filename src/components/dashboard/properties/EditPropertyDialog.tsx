...
import type { Property, PropertyFormData } from "@/hooks/useProperties";

interface Props {
  property: Property | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (id: string, data: Partial<PropertyFormData>) => Promise<boolean>;
}
...
export function EditPropertyDialog({ property, open, onOpenChange, onSubmit }: Props) {
  ...
  useEffect(() => {
    if (property) {
      setForm({
        ...
        avg_nightly_rate: property.avg_nightly_rate,
        pricing_strategy: property.pricing_strategy || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!property) return;
    setLoading(true);
    const ok = await onSubmit(property.id, form);
    setLoading(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5 col-span-2">
              <Label>Nom *</Label>
              <Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required maxLength={200} />
            </div>
            <div className="space-y-1.5 col-span-2">
              <Label>Adresse *</Label>
              <Input value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} required maxLength={500} />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input value={form.pricing_strategy || ""} onChange={e => setForm(p => ({ ...p, pricing_strategy: e.target.value }))} />
          </div>
          <div className="space-y-1.5">
            <Label>Notes</Label>
